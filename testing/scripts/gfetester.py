        ...
        for port in ports:
            ...
            if m:
                if m.group(1) == '1':
                    ...
                    sttyOut = str(subprocess.check_output (f"stty -aF {port.device} | grep min",stderr=subprocess.STDOUT,shell=True),'utf-8')
                    sttyMatch = re.search(r"(^|; )min = (?P<vMin>\d+); time = (?P<vTime>\d+);", sttyOut)
                    if (not sttyMatch):
