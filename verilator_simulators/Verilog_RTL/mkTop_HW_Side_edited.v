...
//
// On Mon Feb  3 22:10:36 GMT 2020
//
//
// Ports:
// Name                         I/O  size props
...
module mkTop_HW_Side(CLK,
  ...
  // submodule soc_top
  assign soc_top$put_from_console_put = v__h957 ;
  ...
  begin
    ...
    v__h756 = v__h762 / 32'd10;
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_terminate)
	c_end_timing({ 32'd0, v__h756 });
    if (RST_N != `BSV_RESET_VALUE)
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_step0)
	begin
	  tohost_addr__h571 = c_get_symbol_val("tohost");
	  #0;
	end
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_step0)
    ...
    if (RST_N != `BSV_RESET_VALUE)
      if (WILL_FIRE_RL_rl_step0) c_start_timing({ 32'd0, v__h627 });
  end
  // synopsys translate_on
