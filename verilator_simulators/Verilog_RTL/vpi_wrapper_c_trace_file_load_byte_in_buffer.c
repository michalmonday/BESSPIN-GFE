 ...
 *     #include "vpi_wrapper_c_trace_file_load_byte_in_buffer.h"
 *     void (*vlog_startup_routines[])() = { c_trace_file_load_byte_in_buffer_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_trace_file_load_byte_in_buffer call=c_trace_file_load_byte_in_buffer_calltf size=32 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_trace_file_load_byte_in_buffer vpiSysFuncSized 32 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
unsigned int c_trace_file_load_byte_in_buffer(unsigned int , char );

/* VPI wrapper function */
PLI_INT32 c_trace_file_load_byte_in_buffer_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  unsigned int arg_1;
  char arg_2;
  ...
  vpi_result = c_trace_file_load_byte_in_buffer(arg_1, arg_2);
...
PLI_INT32 c_trace_file_load_byte_in_buffer_sizetf(PLI_BYTE8 *user_data)
{
  return 32u;
}

/* VPI wrapper registration function */
void c_trace_file_load_byte_in_buffer_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_trace_file_load_byte_in_buffer";
  tf_data.calltf = c_trace_file_load_byte_in_buffer_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_trace_file_load_byte_in_buffer_sizetf;
  tf_data.user_data = "$imported_c_trace_file_load_byte_in_buffer";
