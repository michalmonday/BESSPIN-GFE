...
module mkTop_HW_Side(CLK,
  ...
  // submodule soc_top
  assign soc_top$put_from_console_put = v__h858 ;
  ...
  assign soc_top$set_watch_tohost_tohost_addr = tohost_addr__h680 ;
  ...
  begin
    ...
    if (sysRst_Ifc$OUT_RST != `BSV_RESET_VALUE)
      if (!rg_banner_printed)
	begin
	  tohost_addr__h680 = $imported_c_get_symbol_val("tohost");
	  #0;
	end
    if (sysRst_Ifc$OUT_RST != `BSV_RESET_VALUE)
      if (!rg_banner_printed)
	...
	if (WILL_FIRE_RL_rl_relay_console_in && rg_console_in_poll == 12'd0)
	  begin
	    v__h858 = $imported_c_trygetchar(8'hAA);
	    #0;
	  end
  end
  // synopsys translate_on
