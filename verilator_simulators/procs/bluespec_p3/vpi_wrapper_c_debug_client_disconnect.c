 ...
 *     #include "vpi_wrapper_c_debug_client_disconnect.h"
 *     void (*vlog_startup_routines[])() = { c_debug_client_disconnect_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_debug_client_disconnect call=c_debug_client_disconnect_calltf size=8 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_debug_client_disconnect vpiSysFuncSized 8 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
char c_debug_client_disconnect(char );
...
PLI_INT32 c_debug_client_disconnect_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = c_debug_client_disconnect(arg_1);
...
/* sft: $imported_c_debug_client_disconnect vpiSysFuncSized 8 unsigned */

/* tab: $imported_c_debug_client_disconnect call=c_debug_client_disconnect_calltf size=8 acc=rw:%TASK */

PLI_INT32 c_debug_client_disconnect_sizetf(PLI_BYTE8 *user_data)
{
  return 8u;
}
...
void c_debug_client_disconnect_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_debug_client_disconnect";
  tf_data.calltf = c_debug_client_disconnect_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_debug_client_disconnect_sizetf;
  tf_data.user_data = "$imported_c_debug_client_disconnect";
